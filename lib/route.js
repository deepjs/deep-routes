...
"use strict";
	...
	deep.Route.prototype = {
			...
			if (typeof this.original === 'function') {
				...
				while (count < p.length && p[count]) {
					...
					if (p[count] == "/")
						...
						throw deep.errors.Internal('error : missformed route : ', this.original);
				}
				this.tests = tests;
			}
			return this;
		...
		match: function(parts, index, output) {
			//console.log("deep.Route.match : ", parts, index, output);
			if (!parts.forEach) {
				parts = parts.split("/");
				if (parts[0] === '')
					parts.shift();
			...
			// console.log("route match : ", ok, " - ",  this.optional, this.inverse, " - ", catched.join(","));
			if (ok) {
	...
	function analyseVar(p, count, tests, optional, inversed) {
		...
		if (!parser)
			return deep.errors.Internal('error : missformed route : no parser with type : ' + type, p);
		count += 2;
		var start = count;
	...
	function analyseDisjonction(p, count, tests, optional, inversed) {
		...
		while (p[count] == ',' && p[count])
			count = analyseEntry(p, ++count, disjonctions);
		if (p[count] !== ']')
			return deep.errors.Internal('error : missformed route : group bad end : ', p[count], p);
		else
			count++;
	...
	deep.coreUnits.push("req::deep-routes/units/route-dsl");
