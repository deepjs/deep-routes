	...
	var refresh = deep.RouteNode.refresh = function(matched, route, fromHistory){
		...
		.done(function(){
			if(base.root && base.root.emitter)
				base.root.emitter.emit("refreshed", { refreshed:toRelink, route:route, fromHistory:fromHistory });
			else if(base.emitter)
				base.emitter.emit("refreshed", { refreshed:toRelink, route:route, fromHistory:fromHistory });
		});
	};
