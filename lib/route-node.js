...
define(["require", "deepjs/deep"], function(require, deep)
{
		...
		if(entryNode)
		{
			...
			if(!this._is_ocm_)
			{
				...
				if(!deep.isNode)
				{
					...
					var closure = { node:null };
					entry.route = function(route)
					{
						if(!route)
							return closure.node;
						if(route._deep_matched_node_)
						{
						...
						return closure.node.go(route);
					};
					entry.getRoute = function(route)
					{
						if(!closure.node)
							throw deep.errors.Error(500, "you try to  getRoute from a controller that wasn't on screen");
						// console.log("________________entry.route will launch getRoute : ", route);
						return closure.node.get(route);
					};
				}
			}
		}
	...
	deep.RouteNode.prototype = {
		match: function (rest, index, parentMatch)
		{
			...
			if(!deep.isNode)
				match.get = function(route, asArray){
					if(!route)
						return this.route;
					var finalRoute = null;
					...
					switch(route[0])
					{
						case ".." :
							...
							if(route[0] !== "..")
								route.unshift(".");
							if(this.parent)
								finalRoute = this.parent.get(route);
							else
								finalRoute = null;
							break;
						case "." :
			...
			if(!deep.isNode && this.controllerNode && this.controllerNode.value.route)
				this.controllerNode.value.route(match);
			if(!this.router)
				match.output = {};
			else
			{
	...
	var renderNode = function(m)
	{
		...
		if(m.unmatch)
		{
			if(!deep.isNode && m.node.controllerNode && m.node.controllerNode.value.remove)
					...
					if(m.subloads && m.subloads.length > 0)
					{
						...
						for(var i = 0, len = m.subloads.length; i < len; ++i)
						{
							...
							if(subnode.unmatch)
							{
								...
								if(!deep.isNode && subnode.node.controllerNode && subnode.node.controllerNode.value.remove)
