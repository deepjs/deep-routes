...
define(["require", "deepjs/deep"], function(require, deep)
{
	...
	var renderNode = function(m)
	{
		...
		if(m.unmatch)
		{
		 	//console.log("unmatch NODE : ", m.unmatch.value.remove)
			
			if(m.unmatch && typeof m.unmatch.value.remove === 'function')
						m.unmatch.value.remove();
			return null;
		}
			...
			if(refreshed && (refreshed.then || refreshed.promise))
			{
				...
				.done(function(refreshed){
					if(refreshed)
						...
						console.log("refreshed : ", controllerNode.path, "(nothing) - with : ", m.output);
					...
					if(m.subloads && m.subloads.length > 0)
					{
						...
						for(var i = 0, len = m.subloads.length; i < len; ++i)
						{
							...
							if(subnode.unmatch)
							{
								//console.log("m unmatch : ", subnode.unmatch.value.remove);
								if(subnode.unmatch && typeof subnode.unmatch.value.remove === 'function')
									subnode.unmatch.value.remove();
								continue;
							}
	...
	var refresh = function(matched){
		...
		for(var i = 0; i < len; ++i)
		{
			...
			if(m.unmatch)
			{
				//console.log("m unmatch : ", m.unmatch.value.remove);
				if(m.unmatch && typeof m.unmatch.value.remove === 'function')
						m.unmatch.value.remove();
				continue;
			}
