...
define(["require", "deepjs/deep"], function(require, deep)
{
	...
	var loadNode = function(m)
	{
		...
		{
			var entry = {}, node = m.node.controllerNode.value;
			for(var i in node)
			{
				if(i == 'subs')
					continue;
				entry[i] = deep.utils.copy(node[i]);
			}
			m.node.controllerNode.value = entry;
		}*/
		return deep(m.node.controllerNode)
		.run("load", [{ params:m.output }])
		.done(function(res){
			m.loaded = res;
			return m;
		})
	...
	var refresh = deep.RouteNode.refresh = function(matched, route, fromHistory){
		...
		.always(function(s,e){
			if(base.root && base.root.emitter)
				base.root.emitter.emit("refreshed", { refreshed:toRelink, route:route, fromHistory:fromHistory, error:e });
			else if(base.emitter)
				base.emitter.emit("refreshed", { refreshed:toRelink, route:route, fromHistory:fromHistory, error:e });
		})
	};
