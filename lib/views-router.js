	...
	function(require, deep, structured) {
				...
				.done(function(node) {
					return {
						...
						refresh: function(matched, options) {
							...
							var contextRoute = deep.context("route"),
								self = this;
		...
		return {
			...
			get: function(uri, opt) {
				...
				if (matched.stillToMatch || matched.noMain) {
					var str = matched.stillToMatch ? "stillToMatch" : "noMain";
					//deep.warn("route error : ", str, " : ", matched.path, matched.route, matched.endChild);
					if (this.routeErrorHandler)
						return this.routeErrorHandler(str, matched, opt);
				}
				...
				//.elog("error on views router")
				//.slog("views router success");
			},
			init: function() {
				var self = this;
